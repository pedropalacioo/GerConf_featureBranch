...
export function initEmprestimos() {
  ...
  // Elementos do Modal de Adicionar Empréstimo
  ...
  // Atualiza as opções do select com apenas livros disponíveis
  ...
  function renderEmprestimos() {
    ...
    // Vamos mostrar apenas os empréstimos ativos nesta view para não ficar muito confuso
    ...
    container.innerHTML = ativos.map(emp => /*html*/`
      <div class="book-card bibble-card" data-id="${emp.id}">
        ...
        <div class="card-actions" style="margin-top: 15px;">
          <button class="btn-bibble" style="width: 100%; padding: 8px; font-size: 14px; background: linear-gradient(135deg, var(--bibble-blue-light), white); color: var(--bibble-blue); border: 2px solid var(--bibble-blue);" title="Devolver livro" class="btn-return">
            Devolver Livro
          </button>
        </div>
      </div>
    ...
    document.querySelectorAll('.btn-bibble').forEach(btn => {
      if (btn.innerText === "Devolver Livro") {
  ...
  // ----- EVENTOS MODAL ADICIONAR EMPRÉSTIMO -----
  ...
  formAdd.addEventListener('submit', (e) => {
    ...
    if (!livroIdSelecionado) return; // Segurança

    let livros = getLivros();
    let emprestimos = getEmprestimos();

    // Encontra o livro e marca como indisponível
    const livroIndex = livros.findIndex(l => l.id === livroIdSelecionado);
    if (livroIndex !== -1) {
      livros[livroIndex].disponivel = false;
      salvarLivros(livros); // Salva a alteração do livro no Storage
      
      // Cria o registo de empréstimo
      ...
      salvarEmprestimos(emprestimos); // Salva o novo empréstimo
    }

    renderEmprestimos(); // Atualiza a tela
    modalAdd.classList.add('hidden');
    formAdd.reset();
  });

  // ----- EVENTOS MODAL DEVOLUÇÃO -----
  ...
  btnConfirmReturn.addEventListener('click', () => {
    if (emprestimoIdParaDevolver) {
      ...
      if (empIndex !== -1) {
        ...
        // Atualiza status do livro para disponível novamente
        ...
        renderEmprestimos(); // Atualiza a tela para remover o livro da lista de "Ativos"
      }
